...
import PatternPicker, {
  PatternType,
} from "../../pickers/PatternPicker/PatternPicker";
    ...
    <>
      <div className="flex flex-col gap-2">
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm  font-mono">{shapeStore.strokeSize}px</span>
        </div>
        <Slider
          value={shapeStore.strokeSize}
          onChange={(e: SliderChangeEvent) =>
            shapeStore.updateShape({ strokeSize: e.value as number })
          }
          className="w-full"
          min={0}
          max={100}
...
const NumberOfSidesPicker = () => {
  ...
  return (
    <div>
      <label className="text-sm font-medium">Number of sides</label>
...
const SquareSettings = () => {
  ...
  return (
    <>
      {(shapeStore.fillType === 'outline-only' || shapeStore.fillType === 'outline-and-fill') && <StrokeSlider />}
      <RoundedCorner />
    </>
  );
};
...
const ShapeAdvanceSettings = () => {
  ...
  return (
    <div className="p-4 flex flex-col gap-4 rounded-lg flex-wrap">
